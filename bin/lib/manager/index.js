...
const { README, GITLAB_CI, FINAL_FORM, RECOIL } = require('../constants')
const setupCleaner = require('./setupCleaner')
const dependenciesCleaner = require('./dependencyCleaner')
const removePiStyle = require('./piStyle')
...
const stylingManager = {
  'pi-style': removeMUI,
  mui: async () => (await replaceDefaultWithMUI()) && (await removePiStyle()),
  none: () =>
    Promise.all([removePiStyle(true), removeMUI()]).then((first, ...rest) =>
      first.concat(...rest)
    ),
}
